...
class WooCommerce_Membership_User
{
    ...
    public function __construct($id = null)
    {
        ...
        if (RightPress_Helper::wc_version_gte('3.3.1')) {
            add_filter('woocommerce_get_script_data', array($this, 'enforce_user_registration_js'), 99, 2);
        }
        else {
            add_filter('wc_checkout_params', array($this, 'enforce_user_registration_js_legacy'), 99);
        }
     ...
     * @return array
     */
    public function enforce_user_registration_js($params, $handle)
    {
        if ($handle === 'wc-checkout') {
            $params = $this->enforce_user_registration_js_legacy($params);
        }
     ...
     * @return array
     */
    public function enforce_user_registration_js_legacy($properties)
    {
        // User already registered?
        if (is_user_logged_in()) {
            return $properties;
