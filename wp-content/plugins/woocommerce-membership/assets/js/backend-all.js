...
jQuery(document).ready(function() {
    ...
    function rpwcm_show_hide_access_restriction_fields(update)
    {
        ...
        if (new_value == 'members_with_plans' || new_value == 'users_without_plans') {
            if (update) {
                modify_select2(jQuery('#_rpwcm_only_caps'), 'val', '');
            }
     ...
     */
    function initialize_select2(element, config)
    {
        // Initialize SelectWoo
        if (typeof element.selectWoo !== 'undefined') {
            element.selectWoo(config);
        }
        // Initialize Select2
        else if (typeof RP_Select2 !== 'undefined') {
            RP_Select2.call(element, config);
        }
    }

    /**
     * Modify Select2 on one element
     */
    function modify_select2(element, key, value)
    {
        // Initialize SelectWoo
        if (typeof element.selectWoo !== 'undefined') {
            element.selectWoo(key, value);
        }
        // Initialize Select2
        else if (typeof RP_Select2 !== 'undefined') {
            RP_Select2.call(element, key, value);
        }
    }
