...
class WC_Deposits_Admin_Product{
	...
	public function enqueue_scripts(){
		...
		if( $is_product_editor ){
			wp_enqueue_script( 'wc--deposits-admin-products' , WC_ADVANCE_PLUGIN_URL . '/assets/js/admin/admin-product.js' , array( 'jquery' ) , '' , true );
    ...
    public function add_reminder_meta_box()
    {
        ...
        if ($product) {
            add_meta_box('wc_deposits_second_payment_reminder',
                __('Second Payment Reminder', 'woocommerce-deposits'),
	...
	function variation_override( $loop , $variation_data , $variation ){
		...
		woocommerce_wp_checkbox( array(
			...
			'class' => 'wc_deposits_override_product_settings' ,
	...
	function save_product_variation( $variation_id , $i ){
		...
		$amount_type = ( isset( $_POST[ '_wc_deposits_amount_type' ][ $i ] ) &&
			( $_POST[ '_wc_deposits_amount_type' ][ $i ] === 'fixed' ||
				$_POST[ '_wc_deposits_amount_type' ][ $i ] === 'percent' ) ) ?
			$_POST[ '_wc_deposits_amount_type' ][ $i ] : 'fixed';
