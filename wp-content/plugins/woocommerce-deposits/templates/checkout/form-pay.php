	...
	<table class="shop_table">
		...
		<tbody>
			<?php if ( count( $parent->get_items() ) > 0 ) : ?>
				<?php foreach ( $parent->get_items() as $item_id => $item ) : ?>
					<?php
					if ( ! apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
						continue;
					}
					?>
					<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $parent ) ); ?>">
						...
						<td class="product-subtotal"><?php echo $parent->get_formatted_line_subtotal( $item ); ?></td><?php // @codingStandardsIgnoreLine ?>
					</tr>
				<?php endforeach; ?>
			<?php endif; ?>
		</tbody>
