...
class PAmazonFeed extends PCSVFeedEx
{
    function __construct()
    {
        ...
        $this->addAttributeMapping('Recommended_Browse_Node', 'Recommended Browse Node', false);
        ...
        */
        ...
        //user can specifcy the price rounding by: rule pricerounding(2)
        //$this->addRule('price_rounding','pricerounding',array(2)); //2 decimals
    }
    ...
    function formatProduct($product)
    {
        ...
        if (isset($category[1]))
            $product->attributes['Recommended_Browse_Node'] = trim($category[1]);
        ...
        if (($product->attributes['has_sale_price']) || ( array_key_exists('sale_price', $product->attributes) && $product->attributes['sale_price'] > 0))
