...
class PeBaySellerFeed extends PCSVFeedEx
{
    ...
    function formatProduct($product)
    {
        ...
        if ($pfcore->feedType == 0) {
            ...
            $category = explode(": ", $product->attributes['category']);
        }
        //$category = explode(":", $this->current_category);
        if (isset($category[1]))
...
//featured image / multiple images		
        $product->attributes['PicURL'] = $product->attributes['feature_imgurl'];
        ...
        if ($this->allow_additional_images && (count($product->imgurls) > 0)) {
            ...
            foreach ($product->imgurls as $imgurl) {
                $product->attributes['PicURL'] = $product->attributes['PicURL'] . '|' . $imgurl;
                $image_count++;
                if ($image_count >= 11)
                    break;
            }
