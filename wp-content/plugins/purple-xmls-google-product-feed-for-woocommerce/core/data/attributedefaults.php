...
class PGoogleShipping extends PActionAfterFeed
{
    ...
    {   
        ...
        <g:shipping>' .
            ...
            $this->parent_feed->formatLine('g:price', sprintf($this->parent_feed->currency_format, $product->attributes['shipping_amount']).' '.$product->attributes['currency'], false, '  ') . '
        </g:shipping>';
...
class PTradeMeShipping extends PActionAfterFeed
{
    ...
    public function postProcess($product, &$output)
    {
        ...
        $output .= '
        <tm:shippingOptions><tm:shippingOption>' .
            $this->parent_feed->formatLine('tm:description', $product->attributes['shipping_type'], false, '  ') .
            $this->parent_feed->formatLine('tm:price', sprintf($this->parent_feed->currency_format, $product->attributes['shipping_amount']), false, '  ') . '
        </tm:shippingOption></tm:shippingOptions>';
...
class PSalePriceIfDefined extends PActionBeforeFeed
{
    ...
    public function getValue($item)
    {
        ...
        if (!isset($item->attributes['sale_price'])) {
