...
class PBaseFeedOverride {
	...
	*/
	...
	protected function loadCustomAdvancedCommands($providerName) {
		global $pfcore;
		$loadAdvancedCommands = 'loadCustomAdvancedCommands' . $pfcore->callSuffix;
		$this->$loadAdvancedCommands($providerName);
	}
	...
	private function loadAdvancedCommandsJ($providerName) {
		...
		$db->setQuery('
			...
			WHERE name = ' . $db->quote($providerName . '-cart-product-settings')
		);
		$data = $db->loadResult();
		if (strlen($data) > 0) {
	...
	private function loadCustomAdvancedCommandsW($providerName) {
		$this->advancedCommands = explode("\n", get_option($providerName . '-cart-product-settings-custom'));
	}
