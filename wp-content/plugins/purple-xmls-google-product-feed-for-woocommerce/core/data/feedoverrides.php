...
class PFeedOverride extends PBaseFeedOverride {
	...
	function __construct($providerName, $parent, $saved_feed) {
		...
		if (($saved_feed == null) || (is_object($saved_feed) && $saved_feed->own_overrides != 1)) {
			if ($parent->feedtype == 'Custom') {
		...
		if (count($this->advancedCommands) > 0) {
			foreach ($this->advancedCommands as $this_option) {
				...
				case 'setattributedefault':
					if (isset($params[4])) {
						$defaultClass = 'PActionAfterHarmonize';
					} else {
						$defaultClass = 'PAttributeDefault';
					}
		...
		if (is_array($this->advancedCommandsCustom) && count($this->advancedCommandsCustom) > 0) {
			$limit = '';
			...
			if (is_array($this->advancedCommandsCustom)) {
				foreach ($this->advancedCommandsCustom as $key => $this_option) {
					if (is_array($this_option) && array_key_exists('cpf_feed_type', $this_option) && $this_option['cpf_feed_type'] == 0) {
						$command = 'mapattribute';
					}
					if (is_array($this_option) && array_key_exists('cpf_feed_type', $this_option) && $this_option['cpf_feed_type'] == 1) {
					...
					case 'mapattribute':
						$usesCData = false;
						if (is_array($this_option) && array_key_exists('merchant_attr', $this_option) && !isset($this_option['cpf_feed_value_default'])) {
							$this_option['cpf_feed_value_default'] = $this_option['merchant_attr'];
						}

						$mapattribute = $this_option['cpf_feed_value_default'];
						$defaultClass = 'PAttributeDefault';
						if (is_array($this_option)) {
							$this_option['merchant_attr'] = array_key_exists('merchant_attr', $this_option) ? $this_option['merchant_attr'] : '';
						} else {
							$this_option['merchant_attr'] = '';
						}
	...
	*/
	...
	function interpretSingleSetting($this_option, $parent) {
		...
		if ($this_option == '$no_variation_custom') {
			$parent->no_variation = true;
		}
		if($this_option=='$exclude_parent'){
		    $parent->exclude_parent = true;
        }
        ...
        if($this_option=='$exclude_child'){
		    $parent->exclude_child = true;
        }
		...
		if($this_option=='$remove_miintocategory'){
		    $parent->remove_miintocategory = true;
        }

		if($this_option=='$include_country'){
		    $parent->include_country = true;
        }
