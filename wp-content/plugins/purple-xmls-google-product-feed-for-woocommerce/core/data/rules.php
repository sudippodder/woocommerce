...
class PFeedRuleUpdate extends PFeedRule
{
...
class PFeedRuleUpdatewithrelation extends PFeedRule
{
...
class PFeedRulemapProductAttribute extends PFeedRule
{
...
class PFeedRuleDescription extends PFeedRule
{
    ...
    public function process($product)
    {
        ...
        if (!$this->allow_empty_title) //If description is empty, use title
        {
...
class PFeedRuleCustomStringCase extends PFeedRule
{
    ...
    public function process($product)
    {
        if (isset($this->parametersV[0])) {
            if ($product->attributes[$this->parametersV[0]]) {
                if (strlen($this->parametersV[1]) < 15) {
                    $attributeValue = str_replace("'", '', $product->attributes[$this->parametersV[0]]);
                    $code = ('return ' . $this->parametersV[1] . '("' . $attributeValue . '");');
                    $returnval = $this->evaluate($code, $product->attributes[$this->parametersV[0]],$this->parametersV[1]);
                    $product->attributes[$this->parametersV[0]] = $returnval;
                }
            }
        }
    }

    private function evaluate($code, $original, $function)
    {
        try {
            return eval($code);
        } catch (ParseError $error) {
            $nextCode = 'return ' . $function .'("' .str_replace('"', '', $original) . '");';
            return eval($nextCode);
        }
    }
...
Class PFeedRuleWeightUnits extends PFeedRule{
    public function process($product)
    {
...
class PFeedRuleSetCustomProce extends PFeedRule
{
    //csvstandard should also elminiate characters outside \x20-\x7E ...what about other language chars? -cg
    public function initialize()
    {
...
class PFeedRuleApplyVatOn extends PFeedRule
{
    //csvstandard should also elminiate characters outside \x20-\x7E ...what about other language chars? -cg
    public function initialize()
    {
...
class PFeedRuleCustomMap extends PFeedRule
{
    //csvstandard should also elminiate characters outside \x20-\x7E ...what about other language chars? -cg
    public function initialize()
    {
...
class PFeedRuleSetVatRate extends PFeedRule
{
    //csvstandard should also elminiate characters outside \x20-\x7E ...what about other language chars? -cg
    public function initialize()
    {
...
class PFeedRuleSetCustomCurrency extends PFeedRule
{
    //csvstandard should also elminiate characters outside \x20-\x7E ...what about other language chars? -cg
    public function initialize()
    {
