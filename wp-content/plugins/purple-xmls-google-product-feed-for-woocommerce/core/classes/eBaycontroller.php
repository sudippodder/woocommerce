...
class eBayController
{
    ...
    public function initEbay($site_id, $sandbox_enabled, $token = false, $account_id = false)
    {
        ...
        if (isset($_REQUEST['page']) && sanitize_text_field($_REQUEST['page']) === 'wplister-tools') {
    ...
    public function newAccount($ebay_token)
    {
        ...
        $account->site_id = sanitize_text_field($_REQUEST['site_id']);
        $account->site_code = EbayController::getEbaySiteCode($account->site_id);
        $account->sandbox_mode = sanitize_text_field($_REQUEST['sandbox']);
    ...
    public function initEC($account_id = null, $site_id = null)
    {
        ...
        if (isset($_REQUEST['site_id']) && isset($_REQUEST['sandbox'])) {
            $ebay_site_id = sanitize_text_field($_REQUEST['site_id']);
            $sandbox_enabled = sanitize_text_field($_REQUEST['sandbox']);
            $ebay_token = '';
        }
