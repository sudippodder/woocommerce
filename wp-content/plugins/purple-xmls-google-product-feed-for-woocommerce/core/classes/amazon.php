...
class Amazon
{
    ...
    function makeDefaultAccount()
    {
        ...
        $wpdb->update($this->_getTable(), ['active' => 1], ['id' => sanitize_text_field($_GET['amazon_account'])]);
    ...
    {
        global $wpdb;
        $market = $wpdb->get_row($wpdb->prepare("SELECT code,url FROM " . $this->_getTable('amazon_markets') . " WHERE id = %s", sanitize_text_field($_POST['market_id'])), ARRAY_A);
    ...
    function addAccount()
    {
        global $wpdb;
        $data = new stdClass;
        $insert['title'] = sanitize_text_field($_POST['cpf_account_title']);
        $insert['market_id'] = sanitize_text_field($_POST['cpf_amazon_market_id']);
        $insert['marketplace_id'] = sanitize_text_field($_POST['cpf_marketplace_id']);
        $insert['merchant_id'] = sanitize_text_field($_POST['cpf_merchant_id']);
        $insert['access_key_id'] = sanitize_text_field($_POST['cpf_access_key_id']);
        $insert['secret_key'] = sanitize_text_field($_POST['cpf_secret_key']);
        $insert['active'] = 0;
        $insert['is_valid'] = 0;
        $insert['market_code'] = sanitize_text_field($_POST['cpf_amazon_market_code']);
    ...
    function submitFeed()
    {
        if ($_GET['id'] > 0) {
            ...
            $sql = "SELECT url FROM " . $wpdb->prefix . "cp_feeds WHERE id = " . sanitize_text_field($_GET['id']);
    ...
    function initAPI($insert, $type, $params = array())
    {
        ...
        switch ($type) {
            ...
            case 'cancel_feed':
                ...
                $request->setFeedSubmissionIdList($idList->withId(sanitize_text_field($_GET['feed_id'])));
            ...
            case 'feed_result':
                ...
                $request->setMerchant($merchant_id);
                $request->setFeedSubmissionId(sanitize_text_field($_POST['feed_submission_id']));
    ...
    {
        $feed_submission_id = sanitize_text_field($_POST['feed_submission_id']);
    ...
    function createTemplate()
    {
        ...
        $country = sanitize_text_field($_POST['template_country']);
    ...
    {
        global $wpdb;
        $delete = $wpdb->delete($this->_getTable('user_template'), ['template_name' => sanitize_text_field($_POST['template'])]);
        echo $delete;
    }
}
