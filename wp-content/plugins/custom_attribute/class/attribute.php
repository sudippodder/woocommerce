 ...
 */
class WC_Custom_Admin_Attributes
{
    ...
    {
        $attribute = array(
            'attribute_label'   => isset($_POST['attribute_label']) ? cattr_clean(stripslashes($_POST['attribute_label'])) : '',
            'attribute_name'    => isset($_POST['attribute_name']) ? custom_sanitize_taxonomy_name(stripslashes($_POST['attribute_name'])) : '',
            'attribute_type'    => isset($_POST['attribute_type']) ? cattr_clean($_POST['attribute_type']) : 'select',
            'attribute_orderby' => isset($_POST['attribute_orderby']) ? cattr_clean($_POST['attribute_orderby']) : '',
        ...
        if (empty($attribute['attribute_name'])) {
            $attribute['attribute_name'] = custom_sanitize_taxonomy_name($attribute['attribute_label']);
        }
    ...
    private static function process_add_attribute()
    {
        ...
        $id = custom_create_attribute($args);
    ...
    private static function process_edit_attribute()
    {
        ...
        $id = custom_update_attribute($attribute_id, $args);

        if (is_wp_error($id)) {
            return $id;
        }

        echo '<div class="updated"><p>' . __('Attribute updated successfully', 'woocommerce') . '</p><p><a href="' . esc_url(admin_url('edit.php?post_type=' . POST . '&amp;page=product_attributes')) . '">' . __('Back to Attributes', 'woocommerce') . '</a></p></div>';
    ...
    private static function process_delete_attribute()
    {
        ...
        return custom_delete_attribute($attribute_id);
    }
    ...
    public static function edit_attribute()
    {
        ...
        $attribute_to_edit = $wpdb->get_row('SELECT attribute_type, attribute_label, attribute_name, attribute_orderby, attribute_public FROM ' . $wpdb->prefix . "custom_attribute_taxonomies WHERE attribute_id = '$edit'");
...
?>
        <div class="wrap woocommerce">
            ...
            ?>
                <form action="edit.php?post_type=<?php echo POST; ?>&amp;page=product_attributes&amp;edit=<?php echo absint($edit); ?>" method="post">
                    <table class="form-table">
                        <tbody>
                             ...
                             */
                            if (custom_has_custom_attribute_types()) {
                            ?>
                                <tr class="form-field form-required">
                                    <th scope="row" valign="top">
                                    ...
                                    <td>
                                        <select name="attribute_type" id="attribute_type">
                                            <?php foreach (custom_get_attribute_types() as $key => $value) : ?>
                                             ...
                                             * @deprecated 2.4.0
                                             */
                                            do_action('customaction_admin_attribute_types');
                                            ?>
                                        </select>
                            ...
                            <?php do_action('customaction_after_edit_attribute_fields'); ?>
                        </tbody>
                    </table>
                        ...
                        <table class="widefat attributes-table wp-list-table ui-sortable" style="width:100%">
                            <thead>
                                <tr>
                                    ...
                                    <?php if (custom_has_custom_attribute_types()) : ?>
                            ...
                            <tbody>
                                <?php
                                if ($attribute_taxonomies = custom_get_attribute_taxonomies()) :
                                    foreach ($attribute_taxonomies as $tax) :
                                ?>
                                        <tr>
                                            <td>
                                                <strong><a href="edit-tags.php?taxonomy=<?php echo esc_html(custom_attribute_taxonomy_name($tax->attribute_name)); ?>&amp;post_type=<?php echo POST; ?>"><?php echo esc_html($tax->attribute_label); ?></a></strong>

                                                <div class="row-actions"><span class="edit"><a href="<?php echo esc_url(add_query_arg('edit', $tax->attribute_id, 'edit.php?post_type=' . POST . '&amp;page=product_attributes')); ?>"><?php esc_html_e('Edit', 'woocommerce'); ?></a> | </span><span class="delete"><a class="delete" href="<?php echo esc_url(wp_nonce_url(add_query_arg('delete', $tax->attribute_id, 'edit.php?post_type=' . POST . '&amp;page=product_attributes'), 'woocommerce-delete-attribute_' . $tax->attribute_id)); ?>"><?php esc_html_e('Delete', 'woocommerce'); ?></a></span></div>
                                            </td>
                                            <td><?php echo esc_html($tax->attribute_name); ?></td>
                                            <?php if (custom_has_custom_attribute_types()) : ?>
                                                <td><?php echo esc_html(custom_get_attribute_type_label($tax->attribute_type)); ?> <?php echo $tax->attribute_public ? esc_html__('(Public)', 'woocommerce') : ''; ?></td>
                                            <?php endif; ?>
                                            <td>
                                                <?php
                                            ...
                                            <td class="attribute-terms">
                                                <?php
                                                $taxonomy = custom_attribute_taxonomy_name($tax->attribute_name);
                                                var_dump([$taxonomy, taxonomy_exists($taxonomy)]);

                                                if (taxonomy_exists($taxonomy)) {

                                                    if ('menu_order' === custom_attribute_orderby($taxonomy)) {
                                                    ...
                                                    switch ($tax->attribute_orderby) {
                                                        case 'name_num':
                                                            usort($terms, '_custom_get_product_terms_name_num_usort_callback');
                                                            break;
                                                        case 'parent':
                                                            usort($terms, '_custom_get_product_terms_parent_usort_callback');
                                                            break;
                                                    }
                                                ...
                                                ?>
                                                <br /><a href="edit-tags.php?taxonomy=<?php echo esc_html(custom_attribute_taxonomy_name($tax->attribute_name)); ?>&amp;post_type=<?php echo POST; ?>" class="configure-terms"><?php esc_html_e('Configure terms', 'woocommerce'); ?></a>
                                            </td>
                                        </tr>
                                    <?php
                                    endforeach;
                ...
                <div id="col-left">
                    <div class="col-wrap">
                        <div class="form-wrap">
                            ...
                            <form action="edit.php?post_type=<?php echo POST; ?>&amp;page=product_attributes" method="post">
                                <?php do_action('customaction_before_add_attribute_fields'); ?>
                                ...
                                <?php do_action('customaction_after_add_attribute_fields'); ?>
